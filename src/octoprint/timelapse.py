...
last_modified_finished = None
last_modified_unrendered = None
...
def get_finished_timelapses():
	...
	global last_modified_finished
	last_modified_finished = max(lm)

	return files


def delete_finished_timelapse(name):
	if util.is_allowed_file(name, ["mpg", "mpeg", "mp4"]):
		timelapse_folder = settings().getBaseFolder("timelapse")
		full_path = os.path.realpath(os.path.join(timelapse_folder, name))
		if full_path.startswith(timelapse_folder) and os.path.exists(full_path):
			os.remove(full_path)

	global last_modified_finished
	last_modified_finished = time.time()
...
def get_unrendered_timelapses():
	...
	with _job_lock:
		...
		result = sorted([util.dict_merge(dict(name=key), finalize_fields(key, value)) for key, value in jobs.items()], key=lambda x: x["name"])

		global last_modified_unrendered
		last_modified_unrendered = max(lm)
	...
	with _cleanup_lock:
		for filename in os.listdir(basedir):
			try:
				...
				global last_modified_unrendered
				last_modified_unrendered = time.time()
			except:
